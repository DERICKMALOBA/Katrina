...
export default function ProductList() {
  ...
  const limit = 52; // Number of products per page

  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/products/productslist?page=${page}&limit=${limit}`);
        if (!response.ok) {
  ...
  return (
    <div className="p-4">
        ...
        {products.map((product) => (
          ...
          >
            ...
            <div className="mt-2 relative">
              ...
              <button
                onClick={() => setWishlist((prev) => ({ ...prev, [product.id || product._id]: !prev[product.id || product._id] }))}
      ...
      <div className="flex justify-center mt-6">
        <button disabled={page === 1} onClick={() => setPage(page - 1)} className="px-4 py-2 bg-gray-300 rounded-lg mr-2">
          Prev
        </button>
        <span className="px-4 py-2">Page {page}</span>
        <button onClick={() => setPage(page + 1)} className="px-4 py-2 bg-gray-300 rounded-lg ml-2">
          Next
        </button>
      </div>
    </div>
